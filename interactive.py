...
## Initialize Dash app for web deployment
external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css']
app = dash.Dash(__name__, external_stylesheets=external_stylesheets)

## load data from CSV as pandas DataFame Object
...
##########################################################################################
def mst(G, sourceNode, targetNode):
    T=nx.minimum_spanning_tree(G)
    selectedNodes = sorted(T.edges(data=True))
    d = {'From':[], 'To':[]}
    d['From'] = selectedNodes
    d['To'] = selectedNodes[1:]
    d["To"].append(selectedNodes[-1])
